...
@onready var day_night_cycle: CanvasModulate = null
@onready var time_indicator_ui: Control = null
@onready var sun_moon_progress: TextureProgressBar = null
@onready var time_label: Label = null
@onready var particle_layer: Node2D = null

var day_cycle_time: float = 0.0
const DAY_CYCLE_DURATION: float = 900.0  # 15 minutes in seconds
...
func _setup_day_night_cycle():
	...
	if not day_night_cycle:
		...
		print("✓ Created DayNightCycle node")
	
	day_cycle_time = 0.0
	day_night_cycle.color = COLOR_DAWN
	print("✓ Day/night cycle initialized - starting at dawn (15 min cycle)")
...
func _setup_time_indicator_ui():
	...
	if sun_moon_texture:
		...
		print("⚠ Warning: sun_to_moon.png not found at res://Resources/Map/sun_to_moon.png")
	...
	print("✓ Time indicator UI created with sun/moon progress bar")
...
func _setup_ambient_particles():
	...
	# Mist appearance
	mist_particles.scale_amount_min = 3.0
	mist_particles.scale_amount_max = 6.0
	...
	# Position particle layer at camera center
	if player:
		particle_layer.global_position = player.global_position
	
	print("✓ Ambient particles created (mist & fireflies)")
...
func _setup_dynamic_shadow():
	...
	var shadow_image = Image.create(64, 128, false, Image.FORMAT_RGBA8)
	shadow_image.fill(Color.TRANSPARENT)
	
	# Draw an ellipse shadow
	for x in range(64):
		for y in range(128):
			var dx = (x - 32.0) / 32.0
			var dy = (y - 64.0) / 64.0
			...
			if dist < 1.0:
				var alpha = (1.0 - dist) * 0.3  # Soft shadow
	...
	dynamic_shadow.offset = Vector2(0, 32)  # Offset from player center
...
func _update_day_night_cycle(delta: float):
	...
	day_night_cycle.color = current_color
