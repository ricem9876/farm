...
func _setup_intro_tutorial():
	"""Initialize the intro tutorial"""
	if IntroTutorial:
		IntroTutorial.setup_for_safehouse(self)
		
		# Only start tutorial if it hasn't been started yet
		if IntroTutorial.current_step == IntroTutorial.TutorialStep.NOT_STARTED:
			IntroTutorial.start_tutorial()
			print("✓ Intro tutorial started (new game)")
		else:
			# Tutorial already in progress - just show current objective
			IntroTutorial._show_current_step_objective()
			print("✓ Intro tutorial resumed at step: ", IntroTutorial.TutorialStep.keys()[IntroTutorial.current_step])
