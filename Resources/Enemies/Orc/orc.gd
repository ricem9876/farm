...
func _ready():
	...
	print("🔥 ORC BOSS SPAWNED with ", max_health, " HP")
...
func _apply_level_scaling():
	...
	print("🔥 ORC BOSS scaled to Farm Level ", farm_level, ": HP=", int(max_health), " (+", int((health_multiplier - 1) * 100), "%), Damage=", snappedf(contact_damage, 0.1), " (+", int((damage_multiplier - 1) * 100), "%)")
	...
	for i in range(get_slide_collision_count()):
		...
		if collider and collider.is_in_group("player"):
			if collider.has_method("take_damage"):
				...
				is_paused = true
				print("💀 ORC dealt ", contact_damage, " damage to player (pausing)")
				break
...
func _on_detection_area_entered(body):
	if body.is_in_group("player") and not is_dead:
		player = body
		is_chasing = true
		print("🔥 ORC detected player - boss fight begins!")
...
func take_damage(amount: float, is_crit: bool = false):
	...
	print("🔥 ORC BOSS took ", amount, " damage. HP: ", current_health, "/", max_health)
...
func _spawn_wolf():
	...
	print("🐺 ORC summoned a wolf at ", spawn_pos)
...
func _drop_loot():
	...
	if player and player.level_system:
		if randf() < player.level_system.luck:
			print("🎲 LUCKY! ORC dropped a second key!")
