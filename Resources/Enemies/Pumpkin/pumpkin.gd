# Pumpkin.gd - Pumpkin enemy with 4-direction movement
	...
	for i in range(get_slide_collision_count()):
		...
		if collider and collider.is_in_group("player"):
			if collider.has_method("take_damage"):
				...
				is_paused = true
				print("Pumpkin dealt ", contact_damage, " contact damage")
				break
...
func _die():
	StatsTracker.record_kill("pumpkin")  # Add this line
	if is_dead:
		return
...
func _drop_loot():
	...
	if player and player.level_system:
		if randf() < player.level_system.luck:
			drop_count = 2
			print("DOUBLE DROPS! 2x pumpkin!")
	
	for i in range(drop_count):
		ItemSpawner.spawn_item("pumpkin", global_position, get_parent())
